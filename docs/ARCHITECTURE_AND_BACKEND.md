...
| **API** | CloudBase/Tencent Edge Function `make-server-16010b6f` — single Hono app (~13k lines in `index.tsx`) |
...
**Important:** Most of the app does **not** read `VITE_CLOUDBASE_API_BASE_URL` / `VITE_CLOUDBASE_PUBLISHABLE_KEY`.

The live CloudBase environment is wired through the autogenerated file:
...
- `src/app/contexts/AuthContext.tsx` (CloudBase/Tencent Auth client)
- Most direct `fetch()` calls across components
- `src/app/utils/module-cache.ts` (catalog, vendor data)

**For a new deployment / fork:** update `utils/tencent/cloudbase.ts` (or refactor to env-based config) so `projectId` and `publicAnonKey` match your CloudBase environment. `.env.example` documents optional `VITE_*` overrides used only in a few places (image thumb size, Stripe helper, Vercel challenge API).
...
Migrations under `cloudbase/migrations/` add normalized tables synced from KV:
...
- `kpay-webhook` (must stay `verify_jwt = false` in `cloudbase/config.toml`)
