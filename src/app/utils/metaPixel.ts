...
const pageViewTrackedPixels = new Set<string>();
const hardenedPixelIds = new Set<string>();
...
function applyMetaPixelHardening(pixelId: string): void {
  ...
  hardenedPixelIds.add(id);
}

function shouldFireMetaEvent(eventKey: string): boolean {
  const now = Date.now();
  const scopedEventKey = `${activePixelId || "none"}:${eventKey}`;
  const last = recentMetaEvents.get(scopedEventKey);
  if (last != null && now - last < META_EVENT_DEDUPE_MS) {
    return false;
  }
  recentMetaEvents.set(scopedEventKey, now);
  return true;
}
...
export function trackMetaPageView(pathname?: string): void {
  if (!activePixelId || pageViewTrackedPixels.has(activePixelId)) return;
  pageViewTrackedPixels.add(activePixelId);
